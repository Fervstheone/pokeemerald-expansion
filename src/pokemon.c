...
STATIC_ASSERT(NUM_SPECIES < (1 << 11), BoxMon_species_TooSmall);
STATIC_ASSERT(NUMBER_OF_MON_TYPES + 1 <= (1 << 5), BoxMon_teraType_TooSmall);
STATIC_ASSERT(ITEMS_COUNT < (1 << 10), BoxMon_heldItem_TooSmall);
STATIC_ASSERT(MAX_LEVEL <= 100, BoxMon_experience_PotentiallTooSmall); // Maximum of ~2 million exp.
STATIC_ASSERT(LAST_BALL < (1 << 6), BoxMon_pokeball_TooSmall);
STATIC_ASSERT(MOVES_COUNT_ALL < (1 << 11), BoxMon_moves_TooSmall);
STATIC_ASSERT(ARRAY_COUNT(sCompressedStatuses) <= (1 << 4), BoxMon_compressedStatus_TooSmall);
STATIC_ASSERT(MAX_LEVEL < (1 << 7), BoxMon_metLevel_TooSmall);
STATIC_ASSERT(NUM_VERSIONS < (1 << 4), BoxMon_metGame_TooSmall);
STATIC_ASSERT(MAX_DYNAMAX_LEVEL < (1 << 4), nBoxMon_dynamaxLevel_TooSmall);
STATIC_ASSERT(MAX_PER_STAT_IVS < (1 << 5), BoxMon_ivs_TooSmall);
...
static UNUSED void EncryptBoxMon(struct BoxPokemon *boxMon)
{
    /*
    u32 i;
...
u32 GetBoxMonData3(struct BoxPokemon *boxMon, s32 field, u8 *data)
{
    ...
    case MON_DATA_MIN_TRAINED_HP:
        retVal = boxMon->minTrainedHP;
        break;
    case MON_DATA_MIN_TRAINED_ATK:
        retVal = boxMon->minTrainedAttack;
        break;
    case MON_DATA_MIN_TRAINED_DEF:
        retVal = boxMon->minTrainedDefense;
        break;
    case MON_DATA_MIN_TRAINED_SPEED:
        retVal = boxMon->minTrainedSpeed;
        break;
    case MON_DATA_MIN_TRAINED_SPATK:
        retVal = boxMon->minTrainedSpAttack;
        break;
    case MON_DATA_MIN_TRAINED_SPDEF:
        retVal = boxMon->minTrainedSpDefense;
        break;
    case MON_DATA_IS_SHADOW:
        retVal = 0; //boxMon->isShadow;
        break;
    case MON_DATA_DYNAMAX_LEVEL:
        retVal = 0; //boxMon->dynamaxLevel;
        break;
    case MON_DATA_GIGANTAMAX_FACTOR:
        retVal = 0; //boxMon->gigantamaxFactor;
        break;
    ...
    //}
        case MON_DATA_STATUS:
            retVal = 0; //UncompressStatus(boxMon->compressedStatus);
            break;
        case MON_DATA_HP_LOST:
            retVal = 0; //boxMon->hpLost;
            break;
    default:
        break;
    }
...
void SetBoxMonData(struct BoxPokemon *boxMon, s32 field, const void *dataArg)
{
    ...
    switch (field)
    {
        ...
        case MON_DATA_HYPER_TRAINED_SPDEF:
            ...
            break;
        case MON_DATA_MIN_TRAINED_HP:
            SET8(boxMon->minTrainedHP);
            break;
        case MON_DATA_MIN_TRAINED_ATK:
            SET8(boxMon->minTrainedAttack);
            break;
        case MON_DATA_MIN_TRAINED_DEF:
            SET8(boxMon->minTrainedDefense);
            break;
        case MON_DATA_MIN_TRAINED_SPEED:
            SET8(boxMon->minTrainedSpeed);
            break;
        case MON_DATA_MIN_TRAINED_SPATK:
            SET8(boxMon->minTrainedSpAttack);
            break;
        case MON_DATA_MIN_TRAINED_SPDEF:
            SET8(boxMon->minTrainedSpDefense);
            break;
        case MON_DATA_IS_SHADOW:
